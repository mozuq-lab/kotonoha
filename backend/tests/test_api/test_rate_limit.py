...
def mock_ai_client():
    ...
    with (
        ...
        mock_client.convert_text = AsyncMock(return_value=MOCK_CONVERT_RESPONSE)
        mock_client.regenerate_text = AsyncMock(return_value=MOCK_REGENERATE_RESPONSE)
        yield mock_client
...
@pytest.mark.asyncio
async def test_tc001_正常リクエストテスト_制限内(mock_ai_client):
    """
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        ...
        response = await client.post("/api/v1/ai/convert", json=request_body, headers=AUTH_HEADERS)
...
@pytest.mark.asyncio
async def test_tc003_制限リセットテスト(mock_ai_client):
    """
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        ...
        response1 = await client.post("/api/v1/ai/convert", json=request_body, headers=AUTH_HEADERS)
        ...
        response2 = await client.post("/api/v1/ai/convert", json=request_body, headers=AUTH_HEADERS)
...
async def test_tc004_ip別制限テスト(mock_ai_client, monkeypatch):
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        ...
        response_ip_a = await client.post(
            "/api/v1/ai/convert",
            json=request_body,
            headers={**AUTH_HEADERS, "X-Forwarded-For": "192.168.1.100"},
        )
        ...
        response_ip_b = await client.post(
            "/api/v1/ai/convert",
            json=request_body,
            headers={**AUTH_HEADERS, "X-Forwarded-For": "192.168.1.200"},
        )
...
@pytest.mark.asyncio
async def test_tc101_レート制限超過テスト(mock_ai_client):
    """
...
@pytest.mark.asyncio
async def test_tc102_retry_after_ヘッダーテスト(mock_ai_client):
    """
...
@pytest.mark.asyncio
async def test_tc103_エラーレスポンス形式テスト(mock_ai_client):
    """
...
@pytest.mark.asyncio
async def test_tc104_複数エンドポイントでの独立レート制限テスト(mock_ai_client):
    """
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        ...
        response1 = await client.post(
            "/api/v1/ai/convert", json=convert_request, headers=AUTH_HEADERS
        )
        ...
        response2 = await client.post(
            "/api/v1/ai/regenerate", json=regenerate_request, headers=AUTH_HEADERS
        )
...
@pytest.mark.asyncio
async def test_tc201_境界値テスト_ちょうど制限内(mock_ai_client):
    """
...
@pytest.mark.asyncio
async def test_tc202_境界値テスト_制限超過(mock_ai_client):
    """
...
@pytest.mark.asyncio
async def test_tc203_境界値テスト_ウィンドウリセット境界(mock_ai_client):
    """
...
@pytest.mark.asyncio
async def test_tc204_境界値テスト_retry_after値の範囲(mock_ai_client):
    """
...
async def test_tc301_x_forwarded_for_ヘッダーテスト(mock_ai_client):
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        ...
        response1 = await client.post(
            "/api/v1/ai/convert",
            json=request_body,
            headers={**AUTH_HEADERS, "X-Forwarded-For": client_ip},
        )
        assert response1.status_code == 200  # 【確認内容】: 1回目は成功 🔵
...
@pytest.mark.asyncio
async def test_tc302_不正ipフォールバックテスト(mock_ai_client):
    """
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        ...
        response = await client.post(
            "/api/v1/ai/convert",
            json=request_body,
            headers={**AUTH_HEADERS, "X-Forwarded-For": ""},  # 空のIP
        )
...
async def test_tc303_複数x_forwarded_for_ヘッダーテスト(mock_ai_client):
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        ...
        response1 = await client.post(
            "/api/v1/ai/convert",
            json=request_body,
            headers={**AUTH_HEADERS, "X-Forwarded-For": forwarded_for_multiple},
        )
        assert response1.status_code == 200  # 【確認内容】: 1回目は成功 🟡
        ...
        response2 = await client.post(
            "/api/v1/ai/convert",
            json=request_body,
            headers={**AUTH_HEADERS, "X-Forwarded-For": forwarded_for_first},
        )
...
@pytest.mark.asyncio
async def test_tc304_高頻度リクエストテスト(mock_ai_client):
    """
