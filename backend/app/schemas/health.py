...
class HealthResponse(BaseModel):
    ...
    # 🔵 要件定義書（line 96）に基づく
    status: str = Field(..., description="ヘルスチェックステータス", examples=["ok", "degraded"])

    # 【フィールド定義】: データベース接続状態
    # 【データ型】: 文字列（正常時: "connected"）
    # 🔵 要件定義書（line 97）に基づく
    database: str = Field(..., description="データベース接続状態", examples=["connected"])
...
class HealthErrorResponse(BaseModel):
    ...
    # 🔵 要件定義書（line 107）に基づく
    database: str = Field(..., description="データベース接続状態", examples=["disconnected"])
