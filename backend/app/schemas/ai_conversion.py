...
# 前回の変換結果（previous_result）の文字数上限
# AI応答テキストを無制限に受け入れると、DBペイロード肥大化やプロンプト膨張の
# 原因になるため、入力文字列の上限（500文字）より余裕を持たせた1000文字を上限とする。
PREVIOUS_RESULT_MAX_LENGTH: int = 1000
...
ERROR_PREVIOUS_RESULT_MAX_LENGTH: str = (
    f"前回の変換結果は{PREVIOUS_RESULT_MAX_LENGTH}文字以下にしてください"
)
...
class AIRegenerateRequest(BaseModel):
    ...
    def validate_previous_result(cls, v: object) -> str:
        """前回結果のバリデーション（必須・空文字禁止・文字数上限）"""
        trimmed = validate_required_string(
            v, ERROR_PREVIOUS_RESULT_REQUIRED, ERROR_PREVIOUS_RESULT_EMPTY
        )
        if len(trimmed) > PREVIOUS_RESULT_MAX_LENGTH:
            raise ValueError(ERROR_PREVIOUS_RESULT_MAX_LENGTH)
        return trimmed
