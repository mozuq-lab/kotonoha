...
async def create_conversion_log(
    ...
    # データベースに追加
    # コミットは呼び出し元のセッションライフサイクルに一元化する（ここでは行わない）。
    # リクエストスコープのget_dbが管理するセッションであればリクエスト境界でのcommitに、
    # バックグラウンドタスクが開く独立した短命セッションであればタスク側の明示的なcommitに
    # 委ねることで、同一セッション内の後続DB操作との原子性を保つ。
