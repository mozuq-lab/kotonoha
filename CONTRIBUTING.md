...
6. [ブランチ運用とマージの引き金](#ブランチ運用とマージの引き金)
7. [プルリクエスト](#プルリクエスト)
8. [Issue](#issue)
9. [テスト](#テスト)
10. [ドキュメント](#ドキュメント)
...
2026-08-29 決定（是正計画 §7 より）。

- 同時に開くブランチは **2本まで**。セキュリティ対応のブランチは同時1本
- ブランチの寿命は**原則1営業日**。長引く見込みなら作業を分割する
- CI が緑の小さい独立差分は、大きな作業の完了を待たずに先へマージする
- `main` は**日次で push** して「正」を1つに保つ
- 1つの変更の上限は **400行 / 12ファイル**（`git diff --stat` で判定。超えるなら分割）
- **完了条件を満たしたらマージする。「レビュー指摘ゼロ」を条件にしない**
  （指摘ゼロはレビュアーの出力の性質であって、成果物の性質ではない）
- 指摘のトリアージ（**定義の正は `docs/verification-principles.md`「トリアージ」節**。
  ここは要約であり、食い違ったらあちらが正）:
  - P0（**到達経路を示せる**、かつ**実際に赤を見せた**）→ その周で直す。テスト先行
  - P1（到達経路はあるがテストを書けない・実行できない）→ **検査を自作する前に、
    構造を変えて危険を表現不可能にできないかを問う**（既定は「作らない」）。台帳へ
  - P2（到達経路が無い、または動作が変わらない）→ 触らずに台帳へ。
    **2周連続 P2 は「却下」でクローズ**
  - **P0 の格下げ・繰り延べを実装者が単独で決めない。** 直すか、決定として人に出すか
  - **P0 を残したまま「完了」と言わない**
- 台帳: `docs/ledger.md`（旧 [Issue #85「台帳: 既知の問題」](https://github.com/mozuq-lab/kotonoha/issues/85) からファイルへ移行）
  （`[ ]` 未対応 / `[x]` 対応済み / `[-]` 却下）
- 差分レビューは種類の違う **2系統**で行い、入力には「そのステップの差分＋却下済み一覧＋
  台帳の deferred 一覧」を渡す（詳細は `docs/verification-principles.md`）

背景: 2026-08-24 に5本のブランチが同じ6分間に切られて4日以上滞留し、本流が動かないまま
ブランチ上で完璧を目指す圧力がかかった。これが「修正 → レビュー → 再指摘」8周の
環境条件になった（経緯は `docs/articles/why-redaction-fixes-dont-converge.md`）。
...
2. コードレビューで承認を受けること
3. 指摘は P0（再現つき）のみその場で対応し、P1 / P2 は台帳（`docs/ledger.md`。旧 #85）へ送る。
   **「すべての指摘に対応すること」はマージ条件にしない**
   （詳細は[ブランチ運用とマージの引き金](#ブランチ運用とマージの引き金)）
