...
1. `scripts/inventory.sh` を実行し、出力を手元に置く（数字はすべてここから取る。推測で書かない）。
   主 checkout（worktree ではない）で実行する。worktree では `.git` がファイルなので Stop フックの痕跡が常に「なし」になる
...
- `inventory.sh` の「直近 30 日のコミット（製品 / 全体）」。製品（`lib/`・`backend/app` を変えたもの）がゼロの月は、次の文書・検査の PR の前に理由を 1 行
- リリース条件 4 の人の作業（L-51・L-52・L-84〜L-86）が前回から動いたか。動いていなければ 1 行
- 各仕組み（層 2 のコメント、Stop フック、ADR、台帳、この棚卸し）について「今月、判断が変わった実例」を 1 行ずつ。実例ゼロが 2 回続いた仕組みは廃棄候補として台帳へ
...
- 四半期に 1 回（1・4・7・10 月）、文脈を持たないレビュアーに「コードから文書へ」の向きで不足・余剰・見逃しを探させる（2026-09-12 の監査の形）。結果は台帳へ
