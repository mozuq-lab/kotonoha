...
書き方（indent 幅・タグの改行・行順・コメント）を変えても、パーサが返す集合は
変わらないので、**「別の書き方で素通しする」が原理的に起きない**。

| いま見張っている面 | 状態の取り方 |
|---|---|
| Dart の依存 | `pubspec.yaml` を YAML パーサで読む |
| Python の依存 | `backend/requirements*.txt`（**名前のみ。バージョンは見ない**） |
| Android の権限 | `AndroidManifest.xml` を XML パーサで読む |
| iOS の権限キー | `Info.plist` を plist パーサで読む |
| 設定キー | `.env.example` の**全キー**（「秘密らしさ」を名前で判定しない） |

Phase 2 で公開ルート（`app.openapi()` のパス集合）と import 副作用（import smoke）、
Phase 3 で Hive スキーマが加わる。
