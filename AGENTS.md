...
4. **指摘の仕分け**: レビューは縛らない。指摘は既定で捨て、記録しない。直すのは、証拠（赤くなるテスト・スクリーンショット・実測値）で守る約束を破ると示せたものと、数行で局所的に直せるもの（再レビューしない）。修正の差分も、触るパスで最低等級を決める。守る約束に関わらない指摘への修正には、再レビューを付けない。再現していない重大な経路は PR に 1 行だけ書き、持ち越さない。
...
6. **人が決めること**: 外への影響（main への merge・ストア・支払い・秘密・リポジトリ設定・他人の PR）、製品の約束（守る約束・公開文のプライバシーと医療的な表現・データの扱い）、取り返しのつかないデータ移行。それ以外はエージェントが決め、PR 本文の「私が決めたこと」に書く（人は事後に覆す）。
