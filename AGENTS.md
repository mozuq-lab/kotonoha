...
| 出所 | 受入基準・`openspec/specs/` | **過去に実際に起きた欠陥の形** |
| 書き方 | **TDD**（赤 → 緑 → リファクタ） | TDD にならない。欠陥の形をリスト化して当てる |

B の出発点（このプロジェクトで実際に起きた形）は
`docs/verification-principles.md` にある。実効性は本数ではなく mutation kill rate で測る。
