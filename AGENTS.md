...
7. **P0（到達経路を示せて、かつ実際に赤を見せた）のうち、基準を満たすものは直す。** 基準: 結果が最悪（黙って消える・消えるのに残ると告げる・閉じ込め・取り消せない誤操作）× リリース対象の端末で起きる × ふつうの操作か故障 1 つで届く。片方だけなら P0 ではない。基準を満たさない赤は台帳の「受け入れた限界」へ機序ごとに 1 行（既知の機序なら PR 本文の「受け入れ」だけ）。格下げ・繰り延べを実装者が単独で決めない（直すか、決定として人に出すか）。「指摘ゼロ」を完了条件にしない——十分な探索予算のレビューは必ず何かを見つける。レビューは種類の違う 2 系統を当てる（AI レビュアーの盲点は相関する）。ADR を触る差分には「テスト名か対応箇所のコメントで指せる細部が入っていないか、追記になっていないか」も問う（ADR-005 は 3 PR で 55 行まで膨らみ、誰も気づかなかった）。残りは台帳へ
