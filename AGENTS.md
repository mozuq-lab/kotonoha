...
- **ADR-008** 負債ゲートはブロックを状態比較に限る。パターン照合はブロックに使わない（ドラフト）
...
### 負債を作る行為には理由が要る — 機械が止める（ADR-008）

**「決定が要る場面」を検出して ADR の引用を要求するゲートが CI にある。**

やっていることは1つだけ。**正規のパーサでいまの状態を読み、集合にして、
`.debt-gate/allowlist.json` と比べる。** 差分の行は見ないし、git も使わない。

| いま見張っている面 | 状態の取り方 |
|---|---|
| Dart の依存 | `pubspec*.yaml` を YAML パーサ（`dependency_overrides` と**供給元の差し替え**も含む） |
| Python の依存 | `requirements*.txt` を PEP 508 パーサ（**名前のみ。バージョンは見ない**。VCS / 直 URL / editable も面として数える） |
| Android の権限 | `AndroidManifest.xml` を XML パーサ（**flavor ごと**に別の面） |
| iOS の権限キー | `Info.plist` を plist パーサ |
| 設定キー | `*.env.example` の**全キー**（`export` 前置き・小文字も。「秘密らしさ」を名前で判定しない） |

対象ファイルは**パスを列挙せず探索する**。列挙は必ず漏れる。

Phase 2 で公開ルート（`app.openapi()` の `(パス, メソッド)` 集合）と import 副作用
（import smoke）、Phase 3 で Hive スキーマが加わる。

**保証の範囲**（2系統の独立レビューで一度過大に書いて訂正した）:
**上表のフォーマットの内側では、書き方を変えても集合は変わらない**——indent 幅・
タグの改行・属性の改行・行順・コメント・引用符・`export` 前置き・extras・
範囲指定・環境マーカーで実測済み。**「原理的にすべての素通しが起きない」わけではない。**
`ios/Podfile` と `android/**/build.gradle.kts` のネイティブ依存は、Python 側に
正規パーサが無いため**対象外**である（regex で足すと、それは段階4に戻る）。

**止まったときの通し方は2つ。**

1. 該当 ADR を読み、`.debt-gate/allowlist.json` に足す。
   `"<名前>": {"adr": "ADR-00X", "why": "なぜ許可するか1行"}`。
   **`adr` は `docs/adr/` に実在する番号のみ通る**（`ADR-999` は通らない）
2. 該当する ADR が無いなら、**それはその領域の決定が存在しない**ということ。
   実装をやめて ADR を1本作る（`tsumiki:adr-rubber-duck`）

**発火しないもの**: 依存のバージョン更新（dependabot を塞がないため）、
面の削除（Phase 2 の依存削除・Phase 3 の `isFavorite` 削除を塞がないため）。

```
手元で確認   python3 -m scripts.debt_gate.cli check
いまの状態   python3 -m scripts.debt_gate.cli show
検査の検査   python3 -m scripts.debt_gate.cli selftest
```

**`PostToolUse` フックは置かない**（ADR-008）。ブロックしないフックは
読まれない出力を積むだけになるため、検出は CI に寄せてある。
その代わり**編集から検出までは PR まで遅れる**。

**発火するもう1つ**: 面を消したのに許可リストの行が残っていると落ちる。
許可だけ残すと、あとで ADR 無しで再導入できてしまうため。直し方は1行消すだけ。

**このゲートは既知の入口を検出する補助であって、負債の網羅ではない。**
外部送信先は実行時に組み立てられるため静的には閉じず、ブロック対象から外してある。
既存送信先への payload 追加・既存ルートの認証弱体化のような「意味的な変更」も素通しする。
それらは層1（着手前の影響範囲）と層3（差分レビュー）が担う。

### 機械が見ていない負債（人が止めるしかない）

ゲートが**見ていない**次の変更も、該当する ADR を引用すること（無ければ決定から始める）。

- **永続化面の追加** — Hive の `TypeAdapter`・フィールド・box、DB テーブル、
  新しいファイル出力先（Phase 3 で機械化する）
- **モジュールレベルの可変グローバル・副作用の追加** — `import` しただけで資源を作る形
  （Phase 2 で import smoke により機械化する）
- **公開ルート（HTTP 面）の追加**（Phase 2 で OpenAPI により機械化する）
- **外部送信先の追加** — 新しいホストへ送信するコード（**機械化しない**。
  送信先は実行時に組み立てられるため静的には閉じない）
- **ネイティブ依存の追加** — `ios/Podfile`、`android/**/build.gradle.kts`
  （**機械化しない**。正規パーサが無い）

**この節が層1・層3の入力である。** ゲートを縮めた分、ここが受け皿になる。
