...
4. **着手前に触るファイルを列挙して数え、下の索引の未卒業 5 行ごとに「この変更はこの決定に触れるか」を yes/no で PR 本文に書く。** yes の行だけ本文を読む。触る行為が `docs/adr/README.md` の卒業済み行の「関わる行為」に当たれば、その行の却下案もやっていないか見る。どの行にも当たらなければ、実装ではなく決定から始めるかを問う（8 周はこの工程が無かった）
...
7. **P0 は直す。P0 は到達経路を示せて、実際に赤を見せ、かつ基準を満たすもの。** 基準: 結果がこの利用者にとって最悪（黙って消える・消えるのに残ると告げる・誤発報・緊急ボタンへ届かない閉じ込め・取り消せない誤操作。安全側に外れた告知は含めない）× リリース対象（iOS・Android の端末。Web の複数タブ・debug だけ・Web のストレージ無効は含めない）× ふつうの操作か、よくある故障 1 つで届く（故障の重なり・故意の注入は含めない）。経路と赤の片方だけなら P0 ではない。基準を満たさない赤は台帳の「受け入れた限界」へ機序ごとに 1 行（既知の機序なら PR 本文の「受け入れ」だけ）。それ以外の格下げ・繰り延べを実装者が単独で決めない（直すか、決定として人に出すか）。「指摘ゼロ」を完了条件にしない——十分な探索予算のレビューは必ず何かを見つける。レビューは種類の違う 2 系統を当てる（AI レビュアーの盲点は相関する）。ADR を触る差分には「テスト名か対応箇所のコメントで指せる細部が入っていないか、追記になっていないか」も問う（ADR-005 は 3 PR で 55 行まで膨らみ、誰も気づかなかった）。残りは台帳へ
8. **負債を作る 8 行為には理由（ADR の引用）が要る**: 依存の追加（`requirements*.txt` `pubspec.yaml` `pubspec.lock` `pyproject.toml` `Podfile` `build.gradle.kts`）、永続化面（Hive の adapter・field・box ＝ `lib/shared/models/*_adapter.dart` `lib/core/utils/hive_init.dart`、OS バックアップ規則 ＝ `android/…/res/xml/` に backup_rules・data_extraction_rules を足す変更（#122 で消した）、新しいファイル出力先）、秘密を持つ設定キー（`.env.example` `backend/app/config.py`。`URI` / `URL` / `DSN` で終わる名前を含む。workflow の secrets も）、モジュールレベルの可変グローバルと公開ルート（`backend/app/main.py` `routes.py`）、外部送信先（`backend/app/ai/providers.py` `lib/features/ai_conversion/data/`）、モバイル権限（`AndroidManifest.xml` `Info.plist`）、検査・フック・workflow・スクリプトの自作（`.github/workflows/` `.claude/hooks/` `scripts/`）。**機械は止めない**（規律 4 の yes/no を人が書くだけ。ADR-008）
...
ADR が引く層番号: 層 1 前段（着手前。規律 4。**「存在すべきか」はここでしか問えない**）／層 2 機械（import-linter・mypy --strict と CI の検査。**差分の外を見るのはこれだけ**）／層 3 差分レビュー（PR ごと。規律 7）／層 4 反証可能な検査（未割当）／層 5 全体（月 1 の棚卸し）。

## 決定の索引（未卒業 5 本。卒業済み 5 本の索引と赤の見方は `docs/adr/README.md`。列は両方同じにする）
...
| ADR-005 | frontend は 1 概念 1 真実（お気に入りの正は `favoriteProvider`。履歴・定型文のモデルに `isFavorite` を持たせない）。お気に入りは定型文の削除で消えない。永続化の失敗（作り直し・一部の喪失・設定を含む）は利用者に伝える | 各モデルに isFavorite フィールド、定型文削除でお気に入りも消す | 永続化 | Hive スキーマ許可リスト検査（一部）、TC-SYNC-202 |
| ADR-007 | リリース基準: 公式ストアで本公開、初回は AI 変換抜き可、クラッシュ報告はストア標準に依拠（ADR-009）。条件は 4 つで閉じ、足すには本 ADR の改訂が要る。iOS 先行、Android は Play のクローズドテスト（12 人 × 14 日）通過後 | 日付を置く、自発的な β 配布、組織アカウント（D-U-N-S）で Play の要件を外す | 権限, 外部送信先 | 無し（リリースまで生きる） |
...
パスに現れない再提案（同じ概念を UI ロジックだけで 2 つ目に実装する等）と、規律 8 の「新しいファイル出力先」「`URI`/`URL`/`DSN` で終わる名前」は、この索引の「関わる行為」に翻訳できず拾えない。月 1 の棚卸し（観点 6）が受け皿。

## 文書（ADR-010）

核はこの 1 本。決定は `docs/adr/`（各 60 行、未卒業 5 本）。**正本は読者ごとに 1 つで、同じ内容を 2 箇所に書かない**: 開発者は README、利用者・審査者は `docs/privacy-policy.md` と `docs/support.md`、仕様は `docs/spec/kotonoha-requirements.md`。API の契約テスト基準は `backend/tests/contract/openapi_baseline.json` であり、API 仕様の正本ではない。
...
台帳 `docs/ledger.md` の「判断待ち」を読む。リリースに残るのは ADR-007 条件 4 だけ（`grep 'ADR-007 条件 4' docs/ledger.md` で引ける。開発者登録 L-84 → Play のクローズドテスト L-145 → 審査、が Android の律速）。是正計画（Phase 0〜5）は Phase 5 の完了で終わり、`docs/plans/` から破棄される。
