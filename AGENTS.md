...
| ADR-005 | frontend は 1 概念 1 真実（お気に入りの正は `favoriteProvider`。履歴・定型文のモデルに `isFavorite` を持たせない）。お気に入りは定型文の削除で消えない。永続化の失敗（作り直し・設定を含む）は利用者に伝える | 各モデルに isFavorite フィールド、定型文削除でお気に入りも消す | 永続化 | Hive スキーマ許可リスト検査（一部）、TC-SYNC-202 |
