...
**完了条件**:
- Gitリポジトリが作成され、基本的なディレクトリ構造が存在する
- README.mdにプロジェクト概要とセットアップ手順が記載されている
- `.gitignore`で不要なファイルが除外されている
- 環境変数テンプレート（.env.example）が存在する
...
**完了条件**:
- database-schema.sqlが作成されている
- ERD（Mermaid形式）が作成されている
- テーブル説明書が存在する
- SQLファイルがPostgreSQLで実行可能（文法エラーなし）
...
- ディレクトリ構造が作成されている
- 定数ファイルが実装されている
- 各ディレクトリにREADME.md（説明）が存在する
- Flutter analyzeでエラーがない
