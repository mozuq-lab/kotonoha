...
AI変換機能（オプション）を使用する場合のみ、以下のデータが一時的にサーバーに送信されます。
...
**本アプリは、クラッシュ情報を含むいかなるデータも、本アプリ自身の仕組みで外部へ送信しません。**
...
この仕組みで提供される情報に、**文字盤で入力した内容・定型文・履歴・お気に入りなど、
あなたが入力した内容は含まれません。** 含まれるのは、アプリが停止した箇所を示す
技術情報（スタックトレース）、端末の機種名、OSのバージョン、アプリのバージョンです。
...
Only when using the AI conversion feature (optional), the following data is temporarily sent to our servers:
...
**The App does not transmit any data, including crash information, through any mechanism of
its own.** No crash reporting SDK (such as Firebase Crashlytics or Sentry) is embedded in the App.
...
**Information provided through this mechanism does not include anything you have entered**,
such as text typed on the character board, preset phrases, history, or favorites. It contains
technical information indicating where the app stopped (a stack trace), the device model,
the OS version, and the app version.
...
The App does not share users' personal information with third parties except as required by law.
