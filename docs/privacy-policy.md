...
- **入力履歴**: 文字盤やキーボードで入力し、読み上げた内容など（最大50件）
- **入力中の文**: 文字盤やキーボードで入力している途中の内容。アプリが終了しても入力が消えないように保存します。入力欄を空にすると削除されます
...
ことのは backend は、入力テキスト（再変換時は前回の変換結果も含む）と、選択した丁寧さレベルに応じた指示を prompt に含め、外部 AI provider である OpenAI（米国）へ送信します。provider が生成した変換結果は、ことのは backend を経由して本アプリへ返されます。
...
- **Input History**: Content entered via the character board or keyboard and spoken, among other entries (up to 50 entries)
...
The Kotonoha backend includes the Input Text (and the Previous Conversion Result when regenerating) and instructions based on the selected Politeness Level in a prompt sent to the external AI provider, OpenAI (United States). The provider's Conversion Result is returned to the App through the Kotonoha backend.
