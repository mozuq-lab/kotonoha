...
- 会話内容は端末内のみ（NFR-101）。利用者が同意した AI 変換で送る分を除き、送信しない
- backend は AI 変換の中継だけに使う（ADR-007 は 2026-09-26 に AI 変換を初回に含めた）。クラッシュの送信先には使わない
...
- **D（自前）却下**: backend は初回からあるが（ADR-007、2026-09-26）、送信先を 1 つ増やすことに変わりはない。B と同じく、**送信経路を持たない案が構造的に強い**
