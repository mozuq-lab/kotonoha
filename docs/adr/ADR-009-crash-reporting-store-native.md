...
- **「会話内容・入力内容・ハッシュを一切含まない」は、送信経路を持たないという構造そのもので担保する。** 新しい型・検査・サニタイザは作らない（ADR-008 の教訓: 到達経路を示せない脅威に機械を作らない）。OS のバックアップ機能による複製は本 ADR の送信経路ではない（NFR-106、2026-09-12）
...
(i) 着手時 — 依存・外部送信先（`pubspec.yaml` に触れる変更は A 級。着手時に本 ADR の却下した案と照らす）。
`pubspec.yaml` に crashlytics / sentry / firebase 系の依存が入っていないことは監査（ADR-010）の目視で見る。**機械のゲートは作らない**（ADR-008／AGENTS.md「判断の手順」7）。受け皿は依存・外部送信先を A 級にする等級の一覧と、差分レビュー・監査。ADR-001（ユーザー由来の内容を載せない）は送信経路が無いので破られる面が存在しない。
