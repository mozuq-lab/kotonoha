...
状態: 承認済み（2026-08-29 決定、2026-09-06 書き直し、2026-09-13 改訂: お気に入りは定型文の寿命と独立／作り直しと設定の失敗も伝える。同日、挙動の細部をテストと対応箇所のコメントへ降ろした。2026-09-19 改訂: 破損で一部だけ失ったときも伝える。署名: mozuq）／ 実装は PR #88、#125〜#127、#138〜#140
...
- 永続化は案 c。保存されない状態（書き込み失敗・破損で全部または一部を失った・設定の保存失敗）を `PersistenceState` の型で持ち、利用者に伝えて継続する。退避したデータを戻す手段は持たない（端末内に残すのは、支援者が端末を調べるときの保険）
...
Hive が報告しない書き込み失敗（box が開いたままのディスクフル。hive 2.2.3 `box_impl.dart:82`）は伝えられない。容量不足の警告は要件から外した（2026-09-24）。設定・下書き・チュートリアル完了フラグの保存失敗は同じセッション内でしか伝えられない（保存先が壊れている以上、再起動後に戻った理由を残せない）。同文のお気に入りが 2 件並び得る（L-99）。
