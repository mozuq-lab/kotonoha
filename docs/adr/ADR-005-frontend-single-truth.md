...
- 永続化は案 c。保存されない状態（書き込み失敗・破損で空に作り直した・設定の保存失敗）を `PersistenceState` の型で持ち、利用者に伝えて継続する。退避したデータを戻す手段は持たない（端末内に残すのは、支援者が端末を調べるときの保険）
...
Hive が報告しない書き込み失敗（box が開いたままのディスクフル。hive 2.2.3 `box_impl.dart:82`）は伝えられない。EDGE-003（容量不足の警告）は未達のまま受け入れる。設定の保存失敗は同じセッション内でしか伝えられず（保存先が壊れている以上、再起動後に設定が戻った理由を残せない）、報告経路に乗らない書き込みが残る（L-104）。同文のお気に入りが 2 件並び得る（L-99）。
