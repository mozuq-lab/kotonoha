...
- **信頼するプロキシ**: `TRUSTED_PROXY_COUNT` で、リバースプロキシ経由時に `X-Forwarded-For` のどの位置をクライアントIPとして採用するかを設定（0の場合は接続元IPをそのまま使用）
  - N（≧1）を設定した場合、XFFの**右からN番目**（＝信頼プロキシが観測したクライアントIP）を採用する。クライアントが左側に付与した値は無視される
  - XFFのチェーンがN要素に満たない場合は、信頼プロキシが付与した値が存在しないため**XFFを採用せず接続元IPへフォールバックする**（フェイルクローズ）。最左の値へフォールバックすると、ヘッダーを1つ付けるだけでレート制限を回避できてしまうため
  - **過大設定の危険**: 実際のプロキシ段数より大きいNを設定すると、XFFが常に不採用となり全リクエストがプロキシの接続元IPに収束する。レート制限が全ユーザーで共有され実質的なサービス停止になるため、本番では実構成の段数と一致させること
