...
- [x] L-110 緊急ボタンの E2E（`large_emergency_buttons_test.dart` TC-E2E-084-021）が headless web で時々「はい」の直後に緊急画面を見つけられない（#133 の CI run 34997386337。main の直近 3 回は緑）。`startEmergency()` が音声再生の await の後に `alertActive` を書くため、再生が遅いと緊急表示が遅れる。視覚を先（state を先に書く）にして音声を後にすれば利用者にも E2E にも効くが、決定として出す — frontend/kotonoha_app/lib/features/emergency/presentation/providers/emergency_state_provider.dart:71-82。解消（2026-09-19、視覚を先に決定。state を先に書き、再生の await 中にリセットされたら鳴り始めた音を止める。再生を 3 秒遅らせた E2E で赤→緑）
