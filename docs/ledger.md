...
- [ ] L-197 定型文の保存の待機には上限が無い。Hive の書込が返らないと、ダイアログは凍結したまま緊急ボタンの上に残り、閉じる・戻るも止まる。BASE から — phrase_add_dialog.dart・phrase_edit_dialog.dart の `_saving`（Task 2b 最終レビュー、監査 1 の未確認事項）。決定（2026-09-23、仕分け）: 直す（AGENTS.md の最悪の結果に当たる。上限の値と、上限の後に遅れて成功した保存の扱いは実装時に決める）
