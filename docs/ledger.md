...
- [ ] L-197 定型文の保存の待機には上限が無い。Hive の書込が返らないと、ダイアログは凍結したまま緊急ボタンの上に残り、閉じる・戻るも止まる — phrase_add_dialog.dart・phrase_edit_dialog.dart の `_saving`。決定（2026-09-23）: 直す（守る約束 ①「緊急が確実に届く」を破り得る。上限の値と、上限の後に遅れて成功した保存の扱いは実装時に決める）
