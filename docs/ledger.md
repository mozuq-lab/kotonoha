...
- [ ] L-111 フォント設定の追従はテーマの倍率がけで実装したが、これは「テーマに明示サイズがあるスタイル」（bodyLarge・bodyMedium・titleLarge とボタン）にしか効かない。サイズ無しのスタイル（ダイアログの見出し headlineSmall、bodySmall 等）は Theme.of の localize で既定サイズが後から入るため倍率の外で、REQ-2007「すべてのテキスト要素」は満たしていない。加えて別機構の欠落もある: `emergency_confirmation_dialog.dart:137,145,154,194`（タイトル・本文・警告文・「はい」「いいえ」共通の `_buildDialogButton`、緊急確認が最優先）と `ai_conversion_result_dialog.dart:231,291,305,324,340,365,400,419,444`（タイトル・本文・「採用」「再生成」「元の文を使う」）は Theme を経由しない `AppTextStyles.*`（固定 TextStyle の static getter）で描画しており、フォント設定が一切効かない（行番号は棚卸し時点のコードで実測。「はい」「いいえ」は共通ヘルパーの 1 箇所のみ）。上記の「サイズ無しのスタイル」（Theme 経由だが localize 前に既定サイズが無い）とは別機構（Theme 自体を使っていない）で、根治対象に含める。根治は MediaQuery の textScaler をアプリ根で掛け、文字盤等の 3 つの `switch (fontSize)` を撤去する方式（16/20/24 = 20×0.8/1.0/1.2 なので見た目は同じ。約 10 ファイル）。`AppTextStyles` 固定値は Theme 経由の倍率がけの対象にはできないが、OS レベルの textScaler は描画時に別途乗るため根治案で解消する。決定として出す — frontend/kotonoha_app/lib/core/themes/theme_provider.dart、frontend/kotonoha_app/lib/features/emergency/presentation/widgets/emergency_confirmation_dialog.dart:137,145,154,194、frontend/kotonoha_app/lib/features/ai_conversion/presentation/widgets/ai_conversion_result_dialog.dart:231,291,305,324,340,365,400,419,444、Task 2 のリスクレビュー
