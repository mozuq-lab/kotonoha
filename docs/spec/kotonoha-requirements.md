...
- **NFR-302**: システムはアプリクラッシュからの復旧時に、最後の入力状態を復元しなければならない *mvp-requirements-original.md 6-4から推測*
  ...
  - 保存前flush→定型文本体保存→下書き消去が成功してから閉じる。失敗時は入力を保持し、本体成功後の再試行は消去だけにする。消去に失敗しても閉じる手段を残し、閉じたときは下書きを消さない。明示キャンセル/確認済み破棄だけが消去し、システムバックは消さずに閉じる。保存済み定型文は消さない。復元IDと別内容のレコードは上書きせず、同じ内容のレコードが既にあるときは本体へ書かない。
