...
- **NFR-302**: システムはアプリクラッシュからの復旧時に、最後の入力状態を復元しなければならない *mvp-requirements-original.md 6-4から推測*
  - 現在の復元対象は文字盤と定型文の追加フォーム（本文・カテゴリ・固定保存ID）。編集フォームは未対応。追加下書きは専用SharedPreferences mapで他entryと分離する。初期読込の間だけ入力を止め、読込に失敗した後は追加を続けられる（その間は下書きを書かない）。読込/破損/書込失敗を通知し（書込失敗を検出できるのはWeb/Androidのみ）、正常entryは保持する。
  - 追加下書きは変更後400msで書き込み、pausedでは待機せずflushする。400ms待機中・非同期書込中の終了では入力を失い得る。Future完了も物理disk永続化を保証しない。
  - 保存前flush→定型文本体保存→下書き消去が成功してから閉じる。失敗時は入力を保持し、本体成功後の再試行は消去だけにする。消去に失敗しても閉じる手段を残し、閉じたときは下書きを消さない。読み込めていれば、明示キャンセル/確認済み破棄だけが消去し、システムバックは消さずに閉じる。読み込めていない間は保存・キャンセル・破棄のいずれでも下書きに触れずに閉じる。保存済み定型文は消さない。復元IDと別内容のレコードは上書きせず、同じ内容のレコードが既にあるときは本体へ書かない。
...
- **EDGE-003**: ストレージ容量不足時、システムはユーザーに警告を表示し、古い履歴を優先的に削除する提案を行わなければならない *mvp-requirements-original.md 6-4から推測*（未達を受け入れ: Hive が box の書き込み失敗を報告しない。ADR-005 限界、2026-09-13）
...
- **EDGE-105**: AI変換の入力が2文字未満の場合、システムは変換を実行せず、AI変換ボタンを無効化して不足文字数（「あとN文字入力してください」）を表示してもよい *推測*
