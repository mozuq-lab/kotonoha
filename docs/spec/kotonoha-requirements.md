...
- **NFR-302**: システムはアプリクラッシュからの復旧時に、最後の入力状態を復元しなければならない *mvp-requirements-original.md 6-4から推測*
  - 現在の復元対象は文字盤と定型文の追加フォーム（本文・カテゴリ・固定保存ID）。編集フォームは未対応。追加下書きは専用SharedPreferences mapで他entryと分離し、初期読込に成功するまで入力・書込を止める。読込/破損/書込失敗を通知し、正常entryは保持する。
  - 追加下書きは変更後400msで書き込み、pausedでは待機せずflushする。400ms待機中・非同期書込中の終了では入力を失い得る。Future完了も物理disk永続化を保証しない。
  - 保存前flush→定型文本体保存→下書き消去が成功してから閉じる。失敗時は入力を保持し、本体成功後の再試行は消去だけにする。明示キャンセル/確認済み破棄も消去成功を待ち、保存済み定型文は消さない。復元IDと別内容のレコードを上書きしない。
