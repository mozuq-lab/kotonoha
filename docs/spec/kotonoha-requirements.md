...
- **NFR-302**: システムはアプリクラッシュからの復旧時に、最後の入力状態を復元しなければならない *mvp-requirements-original.md 6-4から推測*
  - 復元対象は文字盤の入力中の文と、定型文の追加・編集フォームの本文・カテゴリ。下書きは端末内に保存し、保存が完了する前に終了すると最新の入力は失われ得る。下書きの読込・破損・書込の失敗は利用者に知らせ（書込失敗を検出できるのは Web/Android のみ）、書込や保存に失敗しても入力中の本文は失わず、下書きを読み込めなくても定型文の追加・編集はできる。追加の下書きを復元したとき、その保存先に別の内容の定型文が保存されていれば上書きしない。編集元の定型文が削除されても編集の下書きは残し、利用者が読む・コピーする・明示的に破棄するために到達でき、その下書きから削除済みの定型文を復活させない。下書きの破棄で保存済みの定型文を消さない。Web で複数のタブを開いている場合、この項目の内容は保証しない
