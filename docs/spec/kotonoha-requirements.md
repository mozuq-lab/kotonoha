...
- **NFR-302**: システムはアプリクラッシュからの復旧時に、最後の入力状態を復元しなければならない *mvp-requirements-original.md 6-4から推測*
  - 復元対象は文字盤の入力中の文と、定型文の追加フォームの本文・カテゴリ（編集フォームは未対応）。下書きは端末内に保存し、保存が完了する前に終了すると最新の入力は失われ得る。下書きの読込・破損・書込の失敗は利用者に知らせて入力を残し（書込失敗を検出できるのは Web/Android のみ）、下書きを読み込めなくても定型文の追加はできる。下書きの復元と保存で、保存済みの定型文を上書き・削除しない
