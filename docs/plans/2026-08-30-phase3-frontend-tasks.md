...
| 文書削減の試行（呼び出し側/実装側の分離） | **完了**（下記。方法は機能した。−31行） |
...
- **台帳 L-31 / L-32（2026-08-30 追加）**: 永続化まわりのテストは
  **本番の `initHive()` を通っていない**（`hive_init_test.dart` は中身を再現している）。
  また `persistence_truth_invariant_test.dart`（236行）は文書が禁じた自作 grep 検出器で、
  WP2 が触る領域を「検査」している。**WP2 の着手時に、これを残すかを先に決めること**

---

## 【完了】文書を減らす — 呼び出し側/実装側の分離の試行（2026-08-30）

**結果: 方法は機能した。削減量は小さい**（`docs/verification-principles.md` 469 → 438行、−31）。
予想どおりで、削減量と方法の成否は別に測ってある。
