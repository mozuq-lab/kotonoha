...
**次は WP5（Hive 許可リスト＋analyzer ルール）。ADR-007 条件1の最後の1項目。**

**WP5 に着手する前に読むこと**:

- **ADR-008**（負債ゲートを機械では作らないと決めた ADR）。**WP5 は「検査を作る」作業なので、
  最も再演の危険が高い。** 3周・約4,600行かけて収束せず、発火実績もゼロだった経緯が書いてある
- `docs/verification-principles.md` の **P1 の処方箋**: 「構造を変えて表現不可能にできないか →
  既存の権威の出力を消費できないか → どちらも無ければ記録して受け入れる。
  **自作の検出器は選択肢に入れない**」
- 同 トリアージ節: P0 は「到達経路を示せる」かつ「実際に赤を見せた」の**両方**
- 同 §3 の `testWidgets` × 実 Hive の制約（実 Hive を使うテストは素の `test()` で書く）

**手本が既にある。** WP2 で作った
`test/shared/models/preset_phrase_adapter_backward_compat_test.dart` と
`history_item_adapter_backward_compat_test.dart` は、**legacy adapter で実 box へ書き、
本番 adapter を `override: true` で登録し直して読む**形になっている。
「機械が権威（Hive 本体）の出力を消費する」検査の実例なので、許可リストもこの形に寄せられる。
**`lib/` のソースを文字列照合する検査を書いてはいけない**——WP2 の直前に、
まさにその形の検出器（236行）を規約違反として削除している（台帳 L-32）。

**WP2 から引き継ぐ注意**:

- **台帳 L-31**: 永続化まわりのテストは**本番の `initHive()` を通っていない**
  （`hive_init_test.dart` は `initHive()` を呼ばず中身を再現している）。
  本番の登録経路を通るのは E2E 1本だけ。**許可リストがどの経路を守るのかを先に決めること**
- **台帳 L-40**: 「定型文がお気に入りか」の判定述語が2箇所、`sourceType` のリテラルが4箇所。
  2系統のレビューが独立に到達した指摘
- **台帳 L-39（判断待ち）**: ADR-005 の却下理由「お気に入りは項目の削除と独立に生きるべき」と、
  実装されている連鎖削除が矛盾している。`TC-SYNC-202` がそれを正解として固定しているので
  **全テスト緑はこの不適合を反証しない**。解くには ADR の改訂が要る
