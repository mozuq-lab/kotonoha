...
作成: 2026-08-29 ／ 状態: **Phase 0 完了（2026-08-29）／ Phase 1 は途中。実行順序は 1→3→2（§5）**

- Phase 0: ADR 7本承認・入力バッファ切り分け済み・台帳 #85 起票済み
- Phase 1: **ブランチ拾い出しのみ完了**（Issue #86）。**負債ゲートは一度実装して revert した**
  （`d6f86718973` → `5fdd39e8fd8`）。差分のパターン照合で作ったため、2系統レビューが
  有効な別表記による素通しを7項目中6項目で検出し、**8周と同じ「事例の列挙」の形**だと判明した。
  作り直しの方針は **ADR-008**（状態比較でブロックし、パターン照合は助言。フックは置かない）
