...
- Phase 1: 負債ゲートをフック＋CI に設置（`scripts/debt_gate/`）・ブランチ拾い出しを #86 に一覧化
...
### Phase 1 — 負債ゲートを張り、作業ブランチを処理する（2日）— **完了（2026-08-29）**

> **完了時の実測。** 検出器は `scripts/debt_gate/`（stdlib のみ・46テスト）。フックは
> `.claude/settings.json` の `PostToolUse`、CI は `.github/workflows/debt-gate.yml`
> （**`paths:` フィルタを付けていない**——既存3本は付いており、層2が差分の外を見られなくなるため独立させた）。
>
> **検査自体を検査した結果**: 意図的に `boto3==1.40.0` を1行足したところ、
> フックが exit 2 で差し戻し、`ADR-001` をコメントに書くと通り、削除では発火しなかった。
> CI モードでも同じ差分で exit 1、PR 本文に `ADR-001` を書くと exit 0。
> 7項目すべてを実在ファイルへの現実的な変更で発火させて確認済み。
>
> **設計を2点変えた**（着手前の影響範囲分析で出た）。どちらも「ゲートが自分の計画を塞ぐ」経路である。
> 1. **追加行だけを見る。** 削除で発火させると Phase 2（依存5行の削除）と
>    Phase 3（`isFavorite` の削除）を自分で塞ぐ
> 2. **依存ゲートは新しいパッケージ名だけを見る。** バージョン更新で落とすと
>    dependabot の PR（現在9本）が毎回落ち、ゲートを外す圧力になる
>
> **AST ゲートの較正**: 現行 backend に当てて17件。すべて ADR-004 が対象とする真の違反
> （`settings = Settings()` / `app = FastAPI(...)` / `setup_logging()` / `engine = create_async_engine(...)`）で、
> 偽陽性ゼロ。**偽陽性はゲートを迂回する習慣を作るので、真の見逃しより高くつく**——
> 大文字定数・`Final` 注釈・型エイリアス・allowlist 済み callee（`getLogger` / `APIRouter` ほか）は通す。
> 見逃す形（`CLIENT = httpx.AsyncClient()`）は項目6（外部送信先）が重ねて拾う。
>
> **ブランチの拾い出しは Issue #86。** 台帳 #85 からリンク済み。ブランチは削除していない。
