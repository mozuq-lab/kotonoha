...
| 3 | Hive が開けないとき利用者に通知される。`isFavorite` が存在しない。往復テストが4 feature に存在。**Hive スキーマの許可リスト検査が CI にある。** **入力バッファの件が切り分け済み**（アプリ不具合なら修正済み）。**E2E は残す4経路が CI で緑、外す2本は nightly か削除に振り分け済み。** **`openspec/specs/` に capability が2つ以上あり、`config.yaml` の `context:` に ADR 要約が入っている** |
