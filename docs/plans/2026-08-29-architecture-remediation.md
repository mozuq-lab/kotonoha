...
| 出所 | 受入基準、`openspec/specs/` の delta | **このプロジェクトで実際に起きた欠陥の形** |
...
**B の対象**（このプロジェクトで実際に起きた形。新 backend にも当てる）:

| リスク | 過去に起きた形 | テストの形 |
|---|---|---|
| 設定の組み立て → 実接続 | `%` / `@` を含む値で接続不可 | 記号入りの値で起動 smoke |
| 境界での例外変換 | サードパーティ例外の文字列が漏れる | 例外を注入し、**全シンク**（stdout / stderr / ログ / HTTP ボディ）を観測 |
| 環境ガード | 本番／開発で分岐する箇所 | **両方の環境で**検証する（片方だけ通して本番で破れた前例がある） |
| 起動 | import しただけで資源を作る | `python -c "import app.main"` の出力全体を grep |
...
**テストは Phase 2 と同じ2系統で書く。** frontend 側の B（リスクテスト）の対象:

| リスク | 過去に起きた形 | テストの形 |
|---|---|---|
| 変換層（mapper） | `isFavorite` が往復で消える | **往復テスト**（domain → storage → domain が恒等） |
| 永続化の失敗 | Hive が開けず黙ってインメモリ | 失敗を注入し、**利用者への通知**を検証 |
| 状態の二重表現 | お気に入りが3箇所 | 真実が1つであることの構造検査（Hive 許可リスト） |
