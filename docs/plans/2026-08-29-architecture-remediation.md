...
作成: 2026-08-29 ／ 状態: **Phase 0・Phase 1 完了（2026-08-29）。次は Phase 3（frontend）。実行順序は 1→3→2（§5）**

- Phase 0: ADR 7本承認・入力バッファ切り分け済み・台帳 #85 起票済み
- Phase 1: **完了**。ブランチ拾い出し（Issue #86）＋ 負債ゲート（`701483ced55`）。
  ゲートは一度パターン照合で作って revert し（`d6f86718973` → `5fdd39e8fd8`）、
  **ADR-008 に基づき状態比較で作り直した**。ブロックするのは5項目
  （Dart 依存・Python 依存・Android 権限・iOS 権限キー・設定キー）。
  公開ルートと import 副作用は Phase 2、Hive スキーマは Phase 3 で加わる。
  `PostToolUse` フックは置かない
...
> **2026-08-29: この節のゲート設計は ADR-008 に置き換わった。**
> 下の7項目の表は「**何を検出するか**」としては有効だが、「**どう検出するか**」は
> ADR-008 を正とすること。初回の実装（`d6f86718973`、revert 済み）は
> 差分のパターン照合で作り、2系統レビューが有効な別表記での素通しを
> 7項目中6項目で検出した——4スペース indent の依存追加、タグ改行した plist キー、
> 大文字名の資源生成、`for` ループ内の副作用呼び出し、`add_route` など。
> **1つずつ塞ぐのは8周と同じ形**なので、ブロックする検査は状態比較
> （正規パーサで読む → 集合にする → 前後を比べる）に限定する。
>
> 本節の「フックと CI の二重で張る」「なぜ CI だけでは足りないか」も
> **ADR-008 で覆された**（フックは置かない）。ブロックしないフックは
> 読まれない出力を積むだけになるため。
>
> **ブランチ拾い出し（下記 `fix/backend-production-hardening` の処理）は完了済み。Issue #86。**
...
**決定を列挙するのではなく、決定が要る場面を検出する。** ただしこのゲートは
**既知の入口を検出する補助**であって、負債の網羅ではない——初版は4項目で出力面
（公開ルート・外部送信先）を見ておらず、7項目にした今も、**既存送信先への payload 追加・
既存ルートの認証弱体化・破壊的 migration のような「意味的な変更」は素通しする**。
それらは層1（前段の影響範囲）と層3（差分レビュー）が担う。項目に漏れが出ていないかは
Phase 4 の棚卸しで見直す。「これは存在すべきか」を問う工程が、ここで自動化される。
...
| 1 | **ブロックする検査が CI に入り、状態比較で実装されている**（ADR-008）。その時点で素材が揃う項目——依存の追加・モバイル権限・`.env.example` の秘密キー——について、**正規パーサで読んだ集合の増加を検出し、ADR 引用が無ければ CI が落ちる**。**引用は `docs/adr/` に実在する番号のみ受理する**（`ADR-999` で通らない）。**状態が取得できなかったこと（git 失敗等）と、変化が無かったことを区別し、取得失敗は落とす。****実在の dependabot ブランチ（バージョン更新のみ）で落ちないことを実測済み。****4スペース indent の依存追加・タグ改行した plist キーでも検出できることを実測済み**（＝書き換えに閉じていることの証拠）。`PostToolUse` フックは置かない（ADR-008）。`openspec/changes/` 配下の tracked ファイル禁止が CI に入っている（§5 条件1）。`fix/backend-production-hardening` から拾う対象が **GitHub Issue に一覧化**され、ブランチには以後コミットしない（**削除はしない**——証拠として残す） |
