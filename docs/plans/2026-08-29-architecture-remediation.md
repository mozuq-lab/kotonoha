...
| 002 `memory://` | 「複数インスタンスで動くように」／`memory://` のまま worker を増やす | `redis` 依存を削除 ＋ 起動ガード（プロセス数>1 ∧ in-memory なら fail-fast） | A + C | 1, 2 |
...
書くことは不可能だが、負債を作る行為は上の6項目でほぼ覆える（**網羅の主張ではない**。
初版は4項目で、入口の負債だけを見て出力面——公開ルート・外部送信先——を見ていなかった。
項目に漏れが出ていないかは Phase 4 の棚卸しで見直す）。
...
- 起動ガード — プロセス数>1（`WEB_CONCURRENCY` / uvicorn workers）かつレート制限ストレージが
  in-memory なら起動失敗（ADR-002。「`memory://` のままスケールアウト」はリポジトリ外の変更で
  起きるため、リポジトリ側のゲートでは捕まらない）
...
- 負債ゲートの6項目が、実際に発火する状態にあるか。項目自体に漏れが出ていないか
...
| 1 | **負債ゲートが `PostToolUse` フックと CI の両方に入り、6種類の行為を検出して ADR 引用を要求する。** 意図的に依存を1行足したとき、**フックがその場で止め**、CI でも落ちることを確認済み。`fix/backend-production-hardening` から拾う対象が特定され、ブランチが閉じている |
| 2 | `app/models` `app/crud` `app/db` `alembic` が存在しない。**`requirements.txt` に `sqlalchemy` `alembic` `asyncpg` `psycopg2-binary` `redis` が存在しない。** `grep -rn "str(exc)\|str(e)\|format_exc" app/` が 0件。`import-linter` `mypy --strict` `pytest-randomly` が CI で緑。**モックが外部 SDK 境界のみにあり、自分の関数を patch している箇所が0件。** 記号を含む API キー・プロバイダキーを設定した状態で起動し、`/health` が 200 を返し AI 変換が1往復する smoke が通る。**`AGENTS.md` の API仕様・開発コマンド節が新 backend の実態と一致している（Redis 前提の記述が残っていない）** |
