...
6. **索引は核に 1 決定 1 行**、卒業済みも含める。列は「決めたこと」「却下した案」「関わる行為」「検査」。
   **「決めたこと」には、コードに現れる識別子かパスを 1〜2 個入れる**（例: ADR-005 は「お気に入りの正は `favoriteProvider`。
   履歴・定型文のモデルに `isFavorite` を持たせない」）。(iii) の決定はこの語の一致でしか気づけないので、抽象語だけの行にしない
7. **「検査」節には、どの層で守るかを必ず書く**（2026-09-05 追記）。次の 3 つのどれか:
   (i) 関わる行為（「負債を作る行為」7 項目のどれか）→ 層 2 が拾う／
   (ii) 検査（テスト・lint・gates。書けるなら ADR と同じ PR で入れる）→ CI が守り、ADR は卒業する／
   (iii) どちらも書けない → 「限界」節に「層 3（核の索引）と月 1 の棚卸しでしか守れない」と明記する。
   (iii) は未卒業 5 本の枠を使い続けるので、その数だけが上限になる。いまの 9 本では 005・007・009 が (iii)
...
| **3 核の索引** | AGENTS.md の索引（未卒業 5 本以下＋卒業済み 1 行）。**着手前の影響範囲出しで、未卒業の各行に「この変更はこの決定に触れるか」を yes/no で 1 行ずつ書き、PR 本文に残す。差分レビュー（層 3）にも同じ行を渡し、実装者と別系統で独立に列挙する** | 差分になる前の会話段階。未卒業の決定だけ。「思い出す」ではなく「列挙する」ので、5 本以下なら安定する。no と書いた判断は人が覆せる |

**層 2 の 4 規則**（ADR-008 の坂道を登れなくする）:

1. パターンを発明しない。照合するファイルは AGENTS.md「負債を作る行為」の一覧そのもの。索引側は「関わる行為」を書くだけ。
   **8 つ目の行為が要るときは、`adr-touch.sh` を直すのではなく、先に AGENTS.md の一覧を変える決定を行い、それを写す**
   （例: ADR-008 の違反「検査・フックの自作」は `.github/workflows/`・`.claude/hooks/`・`scripts/` に現れるが、
   7 項目に無い。見逃しの実例が 2 件出るまで足さない）
...
**A3 の (iii) に当たる ADR（UI の構成・状態設計の原則・プロセスの決定）は、その違反がパスに現れないので層 2 では拾えない。**
層 3 と棚卸しだけで守る。「1 ADR に 1 ディレクトリ」を索引に書く案は、広いと壁紙・狭いと漏れで正規表現と同じ坂道なので、
見逃しの実例が 2 件出るまで採らない。
...
あわせて「### 改修に着手する前に」の 1 項目目を次に置き換える（列挙の手順。A4 層 3）:
