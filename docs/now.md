...
## 最短経路（上から順に。終わった行は消す）
「形」の再確認が必要: 緊急呼び出し削除後の利用シナリオ `integration_test/mvp_scenario_test.dart`（6 本）が iPad シミュレータと Android エミュレータ（電話サイズの `Medium_Phone_API_36.1`）で通ること。以前の 7 本は両方で通過済み。エミュレータの Google TTS エンジンが読み上げのたびにクラッシュしていたときは `adb shell pm clear com.google.android.tts` で直った。
