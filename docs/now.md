...
3. ストア提出前の独立監査（L-98）。AI 変換の送信経路（端末 → backend → OpenAI）も対象にする
...
- backend の公開（L-58）: OpenAI 本番プロジェクトの支出上限（ハードリミット・自動チャージ無し）とキー、公開先の決定と契約、端末キー（`AI_API_KEY`）と `API_BASE_URL` の登録
- AI 変換の公開文: 送り先を OpenAI（米国）と明記した同意ダイアログ・プライバシーポリシー・FAQ の承認と、個人情報保護法 28 条（外国にある第三者への提供）の確認。ストアのプライバシー表示とデータセーフティは開発者登録（L-84）の後
