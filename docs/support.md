...
下書きを読み込めなかったときは「再読み込み」を押してください。読み込めない間も定型文の追加はできますが、下書きは保存されません。復元した下書きと同じIDの定型文が別の内容になっている場合は上書きせず入力を残します。必要な内容をコピーしてから、下書きを明示的に破棄できます。同じ内容ですでに保存されていた場合は、その下書きを消して空の追加フォームを開きます。
