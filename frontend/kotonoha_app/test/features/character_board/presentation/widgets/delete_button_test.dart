...
void main() {
  group('DeleteButton - 正常系テスト', () {
    ...
    /// DeleteButtonタップ時にonPressedコールバックが実行されることを確認
    /// 前提条件
    ...
    // 有効状態の削除ボタン表示
    /// enabled: trueの場合、削除ボタンが有効状態で表示されることを確認
    /// 前提条件
    /// なし
    /// 入力
    /// enabled: true
  ...
  group('DeleteButton - 無効状態テスト', () {
    // 無効状態の削除ボタン表示
    /// enabled: falseの場合、削除ボタンが無効状態で表示されることを確認
    /// 前提条件
    /// なし
    /// 入力
    /// enabled: false
    ...
    // 無効状態の削除ボタンタップは無視される
    /// enabled: falseの場合、タップしてもコールバックが実行されないことを確認
    /// 前提条件
    /// なし
    /// 入力
  ...
  group('DeleteButton - サイズ・アクセシビリティテスト', () {
    // 削除ボタンのサイズが44x44px以上
    /// DeleteButtonのタップターゲットが44x44px以上であることを確認
    /// 前提条件
    /// なし
    /// 入力
    /// なし
    ...
    // 削除ボタンにSemanticsラベルが設定されている
    /// DeleteButtonにアクセシビリティ用のSemanticsラベルが設定されていることを確認
    /// 前提条件
    /// なし
    /// 入力
    /// なし
  ...
  group('DeleteButton - 統合テスト（InputBufferNotifierとの連携）', () {
    // 削除ボタンタップで最後の1文字が削除される
    /// 削除ボタンタップ時にInputBufferNotifier.deleteLastCharacterが呼ばれ
    /// 最後の1文字が削除されることを確認
    /// 前提条件
    ...
    // 入力バッファが空の場合、削除ボタンが無効化される
    /// 入力バッファが空の場合、削除ボタンが無効化されることを確認
    /// 前提条件
    ...
    // 入力バッファに文字がある場合、削除ボタンが有効化される
    /// 入力バッファに文字がある場合、削除ボタンが有効化されることを確認
    /// 前提条件
  ...
  group('DeleteButton - エッジケーステスト', () {
    // 連続削除で正しく文字が削除される
    /// 削除ボタンを連続タップしても正しく文字が削除されることを確認
    /// 前提条件
    ...
    // 空バッファで削除ボタンをタップしてもエラーにならない
    /// 空バッファの状態で削除ボタンをタップしてもエラーが発生しないことを確認
    /// 前提条件
  ...
  group('DeleteButton - テーマ対応テスト', () {
    // ライトテーマで削除ボタンが適切に表示される
    /// ライトテーマで削除ボタンが適切な色で表示されることを確認
    /// 前提条件
    /// ライトテーマが適用されている
    /// 入力
    /// なし
    ...
    // ダークテーマで削除ボタンが適切に表示される
    /// ダークテーマで削除ボタンが適切な色で表示されることを確認
    /// 前提条件
    /// ダークテーマが適用されている
    /// 入力
    /// なし
    ...
    // 高コントラストテーマで削除ボタンが適切に表示される
    /// 高コントラストテーマで削除ボタンが適切な色で表示されることを確認
    /// 前提条件
    /// 高コントラストテーマが適用されている
    /// 入力
    /// なし
