...
void main() {
  group('ClearAllButton - 正常系テスト', () {
    ...
    // 全消去ボタンが警告色で表示される
    /// ClearAllButtonが警告色（赤系）で表示されることを確認
    /// 前提条件
    /// なし
    /// 入力
    /// なし
    ...
    // 全消去ボタンタップで確認ダイアログが表示される
    /// ClearAllButtonタップ時に確認ダイアログが表示されることを確認
    /// 前提条件
    ...
    // 有効状態の全消去ボタン表示
    /// enabled: trueの場合、全消去ボタンが有効状態で表示されることを確認
    /// 前提条件
    /// なし
    /// 入力
    /// enabled: true
  ...
  group('ClearAllButton - 無効状態テスト', () {
    // 無効状態の全消去ボタン表示
    /// enabled: falseの場合、全消去ボタンが無効状態で表示されることを確認
    /// 前提条件
    /// なし
    /// 入力
    /// enabled: false
    ...
    // 無効状態の全消去ボタンタップは無視される
    /// enabled: falseの場合、タップしても確認ダイアログが表示されないことを確認
    /// 前提条件
    /// なし
    /// 入力
  ...
  group('ClearAllButton - サイズ・アクセシビリティテスト', () {
    // 全消去ボタンのサイズが44x44px以上
    /// ClearAllButtonのタップターゲットが44x44px以上であることを確認
    /// 前提条件
    /// なし
    /// 入力
    /// なし
    ...
    // 全消去ボタンにSemanticsラベルが設定されている
    /// ClearAllButtonにアクセシビリティ用のSemanticsラベルが設定されていることを確認
    /// 前提条件
    /// なし
    /// 入力
    /// なし
  ...
  group('ClearAllButton - 統合テスト（InputBufferNotifierとの連携）', () {
    // 全消去確認後にバッファがクリアされる
    /// 全消去ボタンタップ→確認ダイアログで「はい」選択後
    /// InputBufferNotifier.clearが呼ばれバッファがクリアされることを確認
    /// 前提条件
    ...
    // 全消去キャンセル後にバッファが変更されない
    /// 全消去ボタンタップ→確認ダイアログで「いいえ」選択後
    /// バッファが変更されないことを確認
    /// 前提条件
    /// InputBufferNotifierに'おはようございます'が設定されている
    /// 入力
    ...
    // 入力バッファが空の場合、全消去ボタンが無効化される
    /// 入力バッファが空の場合、全消去ボタンが無効化されることを確認
    /// 前提条件
    ...
    // 入力バッファに文字がある場合、全消去ボタンが有効化される
    /// 入力バッファに文字がある場合、全消去ボタンが有効化されることを確認
    /// 前提条件
  ...
  group('ClearAllButton - テーマ対応テスト', () {
    // 各テーマで全消去ボタンが警告色で表示される
    /// ライト/ダーク/高コントラストテーマで全消去ボタンが警告色で表示されることを確認
    /// 前提条件
    /// なし
    /// 入力
    /// 各テーマ
