...
/// パフォーマンス重視: 読み上げ開始まで1秒以内
/// エラー耐性: エラー時も基本機能は継続動作
...
/// 要件対応
/// システムは入力欄のテキストをOS標準TTSで読み上げる機能を提供しなければならない
/// システムは読み上げ中の停止・中断機能を提供しなければならない
/// システムは読み上げ速度を「とても遅い」「遅い」「普通」「速い」の4段階から選択できなければならない
/// パフォーマンス要件
/// TTS読み上げ開始までの時間を1秒以内
/// 信頼性要件
/// 重大なエラーでも基本機能（文字盤+読み上げ）は継続動作
class TTSService {
  /// コンストラクタ
  /// パラメータ
  ...
  /// 制約
  /// 空文字列の場合は何もしない（対応）
  /// 読み上げ中に再度呼び出された場合は前の読み上げを停止してから新規読み上げ（対応）
  /// 最大文字数: 1000文字（入力バッファの制限と同じ）
  /// エラーハンドリング
  /// 初期化失敗時: state=error、エラーメッセージ設定
  /// 読み上げ失敗時: state=error、エラーメッセージ設定、アプリは継続
  /// パフォーマンス
  /// 読み上げ開始まで1秒以内を目標
  /// [text] 読み上げるテキスト（空文字列の場合は何もしない）
  Future<void> speak(String text) async {
    // 入力検証: 空文字列チェック（: 空文字列の読み上げ試行）
    ...
    // 読み上げ中断処理: 対応 - 読み上げ中に新しいテキストを読み上げる場合
