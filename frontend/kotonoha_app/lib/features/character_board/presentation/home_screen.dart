  ...
  Future<bool> _ensureAIPrivacyConsent(
    ...
    final accepted = await showDialog<bool>(
          ...
          builder: (dialogContext) => ConfirmationDialog(
            title: 'AI変換の利用確認',
            // App Store 5.1.2(i): 何を・誰に送るかを示す。送り先を変えたら公開文と一緒に直す
            message: 'AI変換を使うと、入力した文章と選んだ丁寧さ（再生成では前回の変換結果も）が、'
                'ことのはのサーバーを経由してCloudflare（米国）のAIサービスへ送信され、'
                '日本国外で処理されることがあります。'
                'ことのはのサーバーは内容を保存しません。Cloudflareでの扱いは同社の規約に従います。'
                '同意できる場合のみ利用してください。',
            cancelLabel: '同意しない',
            confirmLabel: '同意して利用',
