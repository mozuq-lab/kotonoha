...
Future<Box<T>?> openBoxWithRecovery<T>(
  ...
  try {
    ...
    // （ADR-005、L-90・L-102 後半）。開き直しの成否に結びつけると、開き直しに
    // 失敗したときに利用者へ「保存できません」しか伝わらず、以前のデータが
    // 読めなくなったことは黙って進む（次回起動では box が正常に開くので破損も
    // 見つからず、二度と伝わらない）。
    // ここで言えること: 退避は済んでいる（長さまで検証済み）。削除は試みたが、
    // 失敗しても上で握りつぶすので元のファイルが残っている可能性はある。
    // それでも**このセッションで利用者が読めるのは「空」か「開けない」の
    // どちらか**なので、「空で始めた・元のデータは端末内に退避した」は事実。
    // 削除に成功していれば、次回起動では box が正常に開くので破損も見つからず、
    // ここで伝えなければ二度と伝わらない。削除にも失敗して元のファイルが残って
    // いれば、次回起動でまた破損として扱われる（同じ手順をやり直す）。
    onRecreated?.call();
