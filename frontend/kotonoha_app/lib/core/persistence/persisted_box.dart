...
class PersistedBox<T> {
  ...
  /// 起動直後ではなく最初の書き込みに合わせる理由: compact が失敗すると hive は
  /// `_compactionScheduled` を true のままにし、以後の `compact()` は何もせずに
  /// 返る（`storage_backend_vm.dart:147-148`）。起動時に別枠で 1 回呼ぶと、
  /// その 1 回の失敗だけで**削除のたびの掃除（L-119）まで死ぬ**。書き込みの
  /// 出入口に相乗りさせれば、失敗の機会は増えない。
