...
/// 2. 緊急機能の全画面配線（F3 / , , ）
/// 右下に緊急ボタンを常時表示。
/// 緊急状態（alertActive）の時に緊急アラート画面を最前面に重ねる。
...
class _AppShellState extends ConsumerState<AppShell> {
  ...
  /// 緊急状態がalertActiveに遷移したタイミングでOS音量をチェックする
  ...
  Widget build(BuildContext context) {
    // 緊急状態を監視し、alertActive時に緊急画面を最前面に重ねる
    ...
    // normal -> alertActive への遷移を検知し、音量チェックを一度だけ実行する。
